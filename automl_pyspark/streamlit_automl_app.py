...
# Configure execution environment before importing job manager
...
JOBS_DIR = os.path.join(SCRIPT_DIR, "automl_jobs")
RESULTS_DIR = os.path.join(SCRIPT_DIR, "automl_results") 
...
def has_job_results(job_id: str) -> bool:
    """Check if a job has generated results, indicating it completed its work."""
    result_locations = [
        os.path.join(RESULTS_DIR, job_id),
        os.path.join(SCRIPT_DIR, "automl_results", job_id),
        os.path.join(JOBS_DIR, job_id)
    ]
        ...
        if os.path.exists(location):
            ...
            try:
                for root, dirs, files in os.walk(location):
                    for file in files:
                        if file.endswith(('.json', '.csv', '.xlsx', '.png', '.txt', '.py', '.pkl', '.joblib')):
                            # Exclude status and log files
                            if not any(exclude in file.lower() for exclude in ['status', 'log', 'error', 'metadata']):
                                result_files.append(file)
                
                # If we found result files, the job likely completed its work
                if len(result_files) >= 2:  # At least 2 result files indicates completion
                    return True
            except:
                continue
    
    # Also check GCS for results
    try:
        import subprocess
        gcs_results_path = f"gs://rapid_modeler_app/automl_results/{job_id}/"
        result = subprocess.run(["gsutil", "ls", gcs_results_path], 
                              capture_output=True, text=True, timeout=15)
        if result.returncode == 0 and result.stdout.strip():
            # Count non-log files
            files = result.stdout.strip().split('\n')
            result_files = [f for f in files if not any(exclude in f.lower() for exclude in ['log', 'status', 'error'])]
...
def get_job_status(job_id: str) -> str:
    """Get the status of a job with proper priority-based detection."""
    
    # Priority 1: Check if job is actively running in background job manager
    ...
    # Priority 2: Check for job completion markers (including GCS) - HIGHEST PRIORITY
    # This ensures that jobs that completed their work but were cancelled externally still show as completed
    completion_markers = [
        # Local status files
        os.path.join(JOBS_DIR, job_id, f"{job_id}_status.txt"),  # Structured location
        os.path.join(JOBS_DIR, f"{job_id}_status.txt"),          # Compatibility location
        os.path.join(RESULTS_DIR, job_id, f"{job_id}_status.txt"), # Results location
        # Completion marker files
        os.path.join(JOBS_DIR, job_id, "job_completed.txt"),
        os.path.join(RESULTS_DIR, job_id, "job_completed.txt")
    ]
    
    # Also check for GCS completion status if available
    try:
        import subprocess
        
        # Check main status file
        gcs_status_path = f"gs://rapid_modeler_app/automl_jobs/{job_id}/{job_id}_status.txt"
        result = subprocess.run(["gsutil", "cat", gcs_status_path], 
                              capture_output=True, text=True, timeout=10)
        if result.returncode == 0:
            gcs_status = result.stdout.strip().upper()
            if gcs_status in ["COMPLETED", "COMPLETE", "SUCCESS"]:
                return "Completed"
            elif gcs_status in ["FAILED", "FAILURE", "ERROR"]:
                return "Failed"
        
        # Check early completion marker (survives cancellation)
        early_marker_path = f"gs://rapid_modeler_app/automl_jobs/{job_id}/early_completion_marker.txt"
        result = subprocess.run(["gsutil", "cat", early_marker_path], 
                              capture_output=True, text=True, timeout=10)
        if result.returncode == 0:
            marker_status = result.stdout.strip().upper()
            if marker_status in ["COMPLETED", "COMPLETE", "SUCCESS"]:
                return "Completed"
                
        # Check completion timestamp marker
        timestamp_path = f"gs://rapid_modeler_app/automl_jobs/{job_id}/completion_timestamp.txt"
        result = subprocess.run(["gsutil", "ls", timestamp_path], 
                              capture_output=True, text=True, timeout=10)
        if result.returncode == 0:
            return "Completed"  # Timestamp file exists, job completed
            
    except:
        pass  # GCS check failed, continue with local files
    
    for status_file in completion_markers:
            ...
            try:
                with open(status_file, 'r') as f:
                    ...
                    if status in ["COMPLETED", "COMPLETE", "SUCCESS"]:
                        ...
                        # Check if job actually completed before being cancelled
                        if has_job_results(job_id):
                            return "Completed"  # Job completed its work despite being cancelled
                        return "Stopped"
                    elif status in ["SUBMITTED", "SUBMIT"]:
                        return "Running"  # Treat submitted as running until proven otherwise
                    else:
                        return status.title() if status else "Running"
            except Exception:
                continue
    
    # Priority 3: Check if job has results AND a completion marker
    ...
    for output_dir in output_dirs_to_check:
        if os.path.exists(output_dir) and os.path.isdir(output_dir):
            ...
            completion_markers = [
                ...
                os.path.join(output_dir, "job_completed.txt"),
                os.path.join(output_dir, "automl_results.json")
            ]
            
            # Only consider completed if we have explicit completion markers
            for marker in completion_markers:
                if os.path.exists(marker):
                    return "Completed"
    
    # Priority 4: Check if job directory exists but no completion markers
    job_dir = os.path.join(JOBS_DIR, job_id)
    if os.path.exists(job_dir):
        # Job directory exists but no explicit status - likely running
        return "Running"
    
    # Default: If job was just submitted and we can't determine status
    return "Running"
...
def get_job_logs(job_id: str, max_lines: int = 50) -> List[str]:
    ...
    try:
        ...
        log_file_paths = [
            os.path.join(JOBS_DIR, job_id, "logs", "job_execution.log"),
            os.path.join(JOBS_DIR, job_id, f"{job_id}_log.txt"),
            os.path.join(RESULTS_DIR, job_id, f"{job_id}_log.txt")
        ]
...
def get_dataproc_job_info(job_id: str) -> Dict[str, Any]:
    """Get Dataproc job information including batch ID and console links."""
    try:
        # Check if this is a Dataproc job by looking for job metadata
        job_metadata_paths = [
            os.path.join(JOBS_DIR, job_id, f"{job_id}.json"),
            os.path.join("/tmp/automl_jobs", job_id, f"{job_id}.json")
        ]
        
        for metadata_file in job_metadata_paths:
            if os.path.exists(metadata_file):
                try:
                    ...
                    project_id = os.environ.get('GCP_PROJECT_ID', 'atus-prism-dev')
                    ...
                    if batch_id:
                        return {
                            ...
                            'console_url': f"https://console.cloud.google.com/dataproc/batches/{region}/{batch_id}?project={project_id}",
                            'logs_url': f"https://console.cloud.google.com/storage/browser/{temp_bucket}/google-cloud-dataproc-metainfo/",
                            'gcloud_command': f"gcloud dataproc batches describe {batch_id} --region={region} --project={project_id}",
                            'submission_time': metadata.get('submission_time'),
                            'execution_platform': metadata.get('execution_platform', 'dataproc_serverless')
                        }
                except Exception:
                    continue
        
        return {'is_dataproc': False}
        
    except Exception as e:
        return {'is_dataproc': False, 'error': str(e)}

def download_dataproc_results(job_id: str) -> bool:
    """Download Dataproc job results from GCS to local directories for Streamlit display."""
    try:
        # Get Dataproc job information
        dataproc_info = get_dataproc_job_info(job_id)
        if not dataproc_info.get('is_dataproc'):
            return False
        
        # Import GCS client (only when needed)
        try:
            from google.cloud import storage
        except ImportError:
            print(f"⚠️ Google Cloud Storage client not available - cannot download results for {job_id}")
            return False
        
        project_id = dataproc_info['project_id']
        temp_bucket = dataproc_info['temp_bucket']
        ...
        local_jobs_dir = os.path.join(JOBS_DIR, job_id)
        local_results_dir = os.path.join(RESULTS_DIR, job_id)
        os.makedirs(local_jobs_dir, exist_ok=True)
        os.makedirs(local_results_dir, exist_ok=True)
        
        # Define GCS paths to download
        gcs_paths = [
            f"automl_jobs/{job_id}/results/feature_profiling_combined.json",
            f"automl_jobs/{job_id}/results/final_results.json",
            f"automl_jobs/{job_id}/results/model_comparison.json",
            f"automl_jobs/{job_id}/results/model_metrics.json",
            f"automl_jobs/{job_id}/data_analysis/classification_variable_tracking_report.xlsx",
            f"automl_jobs/{job_id}/plots/feature_importance.png",
            f"automl_jobs/{job_id}/logs/automl_main.log"
        ]
        ...
        for gcs_path in gcs_paths:
            try:
                ...
                if blob.exists():
                    ...
                    if gcs_path.endswith('.json') or gcs_path.endswith('.txt'):
                        local_path = os.path.join(local_results_dir, os.path.basename(gcs_path))
                    elif gcs_path.endswith('.xlsx'):
                        local_path = os.path.join(local_results_dir, os.path.basename(gcs_path))
                    elif gcs_path.endswith('.png'):
                        local_path = os.path.join(local_results_dir, os.path.basename(gcs_path))
                    elif gcs_path.endswith('.log'):
                        local_path = os.path.join(local_jobs_dir, os.path.basename(gcs_path))
                    else:
                        local_path = os.path.join(local_results_dir, os.path.basename(gcs_path))
                    ...
                    print(f"📥 Downloaded: gs://{temp_bucket}/{gcs_path} -> {local_path}")
        ...
        if downloaded_files:
            print(f"✅ Downloaded {len(downloaded_files)} files for job {job_id}")
            return True
        else:
            print(f"⚠️ No files found to download for job {job_id}")
            return False
            
    except Exception as e:
        print(f"❌ Error downloading Dataproc results for {job_id}: {e}")
        return False
...
def get_job_logs(job_id: str, max_lines: int = 50) -> List[str]:
    """Get recent job logs with GCS fallback for comprehensive logs."""
    if job_manager is not None:
        return job_manager.get_job_logs(job_id, max_lines)
    else:
        # Try structured log directory first (within automl_jobs)
        structured_log_file = os.path.join(SCRIPT_DIR, 'automl_jobs', job_id, 'logs', 'job_execution.log')
        ...
        # Try to download comprehensive logs from GCS if not available locally
        try:
            import subprocess
            import tempfile
            
            # Try to download comprehensive log from GCS
            gcs_log_path = f"gs://rapid_modeler_app/automl_jobs/{job_id}/logs/job_execution.log"
            
            with tempfile.NamedTemporaryFile(mode='w+', delete=False, suffix='.log') as temp_file:
                temp_log_path = temp_file.name
            
            # Download log from GCS
            download_cmd = ["gsutil", "cp", gcs_log_path, temp_log_path]
        ...
        # Fallback to compatibility log file
...
def get_next_job_number() -> int:
    ...
    # Fallback: Use timestamp-based numbering if file operations fail
    def get_fallback_job_number():
        """Generate a job number based on existing jobs when file operations fail."""
        try:
            existing_jobs = []
            if os.path.exists(JOBS_DIR):
                for item in os.listdir(JOBS_DIR):
                    if item.startswith("job_") and os.path.isdir(os.path.join(JOBS_DIR, item)):
                        # Extract job number from job ID
                        parts = item.split('_')
                        if len(parts) >= 2:
                            try:
                                job_num = int(parts[1])
    ...
    if not lock_acquired:
        st.warning("⚠️ Could not acquire job counter lock. Using fallback numbering.")
        return get_fallback_job_number()
    
    try:
        # Read current counter with robust error handling
        current_number = 0
        if os.path.exists(JOB_COUNTER_FILE):
            try:
                # Use multiple attempts to handle stale file handle issues
                for attempt in range(3):
                    try:
                        ...
                        if attempt < 2:  # Not the last attempt
                            ...
                            st.warning(f"⚠️ Could not read job counter file: {e}. Using default counter.")
                            current_number = 0
                            break
            except Exception as e:
                st.warning(f"⚠️ Error reading job counter: {e}. Using default counter.")
                current_number = 0
        ...
        # Increment and save new counter with robust error handling
            ...
            for attempt in range(3):
                try:
                    ...
                    if attempt < 2:  # Not the last attempt
                        ...
                        # Last attempt failed
                        st.error(f"❌ Could not write job counter file: {e}")
                        return next_number  # Return the number anyway
        except Exception as e:
            st.error(f"❌ Error writing job counter: {e}")
            return next_number  # Return the number anyway
        
        return next_number
    except (ValueError, FileNotFoundError, OSError, IOError) as e:
        # If there's any issue, use fallback numbering
        st.warning(f"⚠️ File operations failed: {e}. Using fallback numbering.")
        return get_fallback_job_number()
    finally:
...
def create_job_submission_page():
                    ...
                    with col1:
                        ...
                        if oot1_table:
                            ...
                            if '.' in oot1_table and len(oot1_table.split('.')) >= 2:
                                ...
                                # Clean any backticks from BigQuery table reference
                                clean_oot1_table = oot1_table.strip('`')
                                oot1_config = {
                                    "source_type": "bigquery",
                                    "data_source": clean_oot1_table,
                    ...
                    with col2:
                        ...
                        if oot2_table:
                            ...
                            if '.' in oot2_table and len(oot2_table.split('.')) >= 2:
                                ...
                                # Clean any backticks from BigQuery table reference
                                clean_oot2_table = oot2_table.strip('`')
                                oot2_config = {
                                    "source_type": "bigquery",
                                    "data_source": clean_oot2_table,
                                    "options": enhanced_data_config.get('options', {}) if enhanced_data_config else {}
                                }
                            else:
                                st.error("❌ Invalid table format. Use: project.dataset.table")
    ...
    with col2:
        ...
        submit_clicked = st.button("🚀 Submit Rapid Modeler Job", type="primary", use_container_width=True)
        
        if submit_clicked and not st.session_state.job_submission_in_progress:
            ...
            # Generate unique job ID (no need to check filesystem anymore)
            ...
            if task_type in ["classification", "regression"]:
                ...
                if oot1_config and oot1_config.get('data_source'):
                    if oot1_config['source_type'] == 'upload':
                        ...
                        # BigQuery table - use table reference directly, clean any backticks
                        oot1_bigquery_table = oot1_config['data_source'].strip('`') if isinstance(oot1_config['data_source'], str) else oot1_config['data_source']
                ...
                if oot2_config and oot2_config.get('data_source'):
                    if oot2_config['source_type'] == 'upload':
                        ...
                        # BigQuery table - use table reference directly, clean any backticks
                        oot2_bigquery_table = oot2_config['data_source'].strip('`') if isinstance(oot2_config['data_source'], str) else oot2_config['data_source']
            ...
            # Submit job with minimal UI feedback (optimized for speed)
            ...
            # Redirect user to Results page for monitoring
            st.info("📊 **Monitor your job progress in the Results page** → Select your job to see live logs and status updates.")
...
def create_results_page():
    ...
    # Also include our enhanced job naming pattern
    all_job_files.extend(glob.glob(os.path.join(JOBS_DIR, "*", "*.json")))
    ...
    result_dirs_to_scan = [
        RESULTS_DIR,
        SCRIPT_DIR,  # Use SCRIPT_DIR instead of "."
    ]
    ...
    for job_file in job_files:
        try:
            with open(job_file, 'r') as f:
                ...
                # For completed Dataproc jobs, try to download results if not already available locally
                if job['status'] == 'Completed' and not output_dir:
                    dataproc_info = get_dataproc_job_info(job_id)
                    if dataproc_info.get('is_dataproc'):
                        print(f"🔄 Attempting to download results for Dataproc job {job_id}...")
                        if download_dataproc_results(job_id):
                            # Re-check for output directory after download
                            for result_base_dir in result_dirs_to_scan:
                                potential_dir = os.path.join(result_base_dir, job_id)
                                if os.path.exists(potential_dir):
                                    job['output_dir'] = potential_dir
                                    break
    ...
    # Display appropriate content based on job status
    if status == 'Running':
        # Enhanced status message with user ID and model name
        user_display = selected_job.get('user_id', 'Unknown')
        model_display = selected_job.get('model_name', 'Unknown')
        st.info(f"🔄 **Job {job_number}** - **{user_display}** - **{model_display}** is currently running...")
        
        # Check if this is a Dataproc job and show GCP links
        dataproc_info = get_dataproc_job_info(job_id)
        if dataproc_info.get('is_dataproc'):
            st.markdown("---")
            st.subheader("☁️ Google Cloud Dataproc Information")
            
            col1, col2 = st.columns([1, 1])
            
            with col1:
                st.info(f"""
                **📊 Dataproc Batch Details:**
                - **Batch ID**: `{dataproc_info.get('batch_id', 'Unknown')}`
                - **Project**: `{dataproc_info.get('project_id', 'Unknown')}`
                - **Region**: `{dataproc_info.get('region', 'Unknown')}`
                - **Platform**: `{dataproc_info.get('execution_platform', 'Unknown')}`
                """)
            
            with col2:
                st.markdown("**🔗 GCP Console Links:**")
                
                # Dataproc Batch Console
                if dataproc_info.get('console_url'):
                    st.markdown(f"🖥️ [**Dataproc Batch Console**]({dataproc_info['console_url']})")
                
                # Storage Browser for Logs
                if dataproc_info.get('logs_url'):
                    st.markdown(f"📋 [**Dataproc Logs (GCS)**]({dataproc_info['logs_url']})")
                
                # Gcloud Command
                if dataproc_info.get('gcloud_command'):
                    st.code(dataproc_info['gcloud_command'], language='bash')
                    st.caption("↑ Copy this command to check job status via gcloud CLI")
                
                # Download Results Button
                st.markdown("---")
                if st.button("📥 **Download Results from GCS**", key=f"download_{job_id}", help="Download job results from Google Cloud Storage to view locally"):
                    with st.spinner("Downloading results from GCS..."):
                        if download_dataproc_results(job_id):
                            st.success("✅ Results downloaded successfully! Refresh the page to see them.")
                            st.rerun()
                        else:
                            st.error("❌ Failed to download results. Check the logs above for details.")
            
            st.success("💡 **Tip**: Use the GCP Console links above for detailed Dataproc monitoring and advanced debugging.")
        ...
        # Add real-time log viewing for running jobs
        st.markdown("---")
        st.subheader("📋 Live Job Logs")
        
        # Get recent logs
        logs = get_job_logs(job_id, max_lines=30)
        if logs:
            # Create a container for logs with auto-refresh
            log_container = st.container()
            with log_container:
                st.info("🔄 **Live logs** (auto-refreshing every 5 seconds)")
                
                # Show logs in a scrollable area
                for log_line in logs[-15:]:  # Show last 15 lines
                    if "✅" in log_line or "completed successfully" in log_line.lower():
                        st.success(log_line)
                    elif "❌" in log_line or "error" in log_line.lower() or "failed" in log_line.lower():
                        st.error(log_line)
                    elif "⚠️" in log_line or "warning" in log_line.lower():
                        st.warning(log_line)
                    elif "🔄" in log_line or "starting" in log_line.lower() or "initializing" in log_line.lower():
                        st.info(log_line)
                    else:
                        st.text(log_line)
            ...
            st.info("📋 No logs available yet... Job may be starting up.")
            # Still auto-refresh to check for new logs
            time.sleep(3)
            st.rerun()
        ...
        # Check if this is a Dataproc job and show GCP links for debugging
        dataproc_info = get_dataproc_job_info(job_id)
        if dataproc_info.get('is_dataproc'):
            st.markdown("---")
            st.subheader("☁️ Google Cloud Dataproc Debugging")
            
            st.warning(f"""
            **🔍 Dataproc Job Failed - Debug Information:**
            - **Batch ID**: `{dataproc_info.get('batch_id', 'Unknown')}`
            - **Project**: `{dataproc_info.get('project_id', 'Unknown')}`
            - **Region**: `{dataproc_info.get('region', 'Unknown')}`
            """)
            ...
            with col1:
                if dataproc_info.get('console_url'):
                    st.markdown(f"🖥️ [**View in Dataproc Console**]({dataproc_info['console_url']})")
                
                if dataproc_info.get('logs_url'):
                    st.markdown(f"📋 [**View Detailed Logs in GCS**]({dataproc_info['logs_url']})")
            
            with col2:
                if dataproc_info.get('gcloud_command'):
                    st.code(dataproc_info['gcloud_command'], language='bash')
                    st.caption("↑ Get detailed job status via gcloud CLI")
        
        # Show logs for failed jobs
        st.subheader("📋 Error Logs")
        logs = get_job_logs(job_id, max_lines=50)
        if logs:
            for log_line in logs[-20:]:  # Show last 20 lines
                if "❌" in log_line or "error" in log_line.lower() or "failed" in log_line.lower():
                    st.error(log_line)
                elif "⚠️" in log_line or "warning" in log_line.lower():
                    st.warning(log_line)
                else:
        ...
        # Show Dataproc information for completed jobs (for reference)
        dataproc_info = get_dataproc_job_info(job_id)
        if dataproc_info.get('is_dataproc'):
            with st.expander("☁️ **Google Cloud Dataproc Details** (Click to expand)", expanded=False):
                col1, col2 = st.columns([1, 1])
                ...
                with col1:
                    ...
                    **📊 Execution Details:**
                    ...
                    - **Project**: `{dataproc_info.get('project_id', 'Unknown')}`
                    - **Region**: `{dataproc_info.get('region', 'Unknown')}`
                    - **Submitted**: `{dataproc_info.get('submission_time', 'Unknown')}`
                    """)
                
                with col2:
                    st.markdown("**🔗 Reference Links:**")
                    if dataproc_info.get('console_url'):
                        st.markdown(f"🖥️ [**Dataproc Console**]({dataproc_info['console_url']})")
                    if dataproc_info.get('logs_url'):
                        st.markdown(f"📋 [**Execution Logs**]({dataproc_info['logs_url']})")
        
        # Continue to show full results below
    else:
        # Check if this job has results despite not being marked as completed
        if has_job_results(job_id):
            st.warning(f"⚠️ Job {job_number} status: {status} (but results are available)")
            st.info("🎯 This job generated results before being interrupted. Showing available results below.")
            # Continue to show results below
        else:
            st.warning(f"⏳ Job {job_number} status: {status}")
            # For submitted/unknown status, show as if running
            st.info("📋 Job may be starting up... Check logs below or refresh the page.")
            
            # Show logs for jobs in unknown state
            logs = get_job_logs(job_id, max_lines=20)
            if logs:
    ...
    if task_type == 'clustering':
        tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["📈 Model Performance", "🔬 Cluster Profiling", "📊 Feature Profiling", "🔍 File Viewer", "💾 Scoring Code", "📜 Logs"])
        ...
        with tab3:
            display_feature_profiling(output_dir, job_id)
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(["📈 Model Performance", "📊 Feature Profiling", "🔍 File Viewer", "💾 Scoring Code", "📜 Logs"])
...
def display_model_performance(output_dir: str, task_type: str = 'classification'):
    ...
    if task_type == 'clustering':
        ...
        with st.spinner("Loading model metrics..."):
            ...
            # Initialize metrics_data to avoid UnboundLocalError
            metrics_data = []
...
def display_logs(output_dir: str, job_id: str):
    ...
    # Add execution script (try organized structure first)
    organized_script = os.path.join(SCRIPT_DIR, 'automl_jobs', job_id, f"{job_id}_script.py")
    compat_script = os.path.join(JOBS_DIR, f"{job_id}_script.py")
    
    if os.path.exists(organized_script):
        log_options.append("🔧 Execution Script (Organized)")
    elif os.path.exists(compat_script):
        log_options.append("🔧 Execution Script (Compatibility)")
    
    # Add job execution log (try structured directory first, then GCS)
    structured_job_log = os.path.join(SCRIPT_DIR, 'automl_jobs', job_id, 'logs', 'job_execution.log')
    compat_job_log = os.path.join(JOBS_DIR, f"{job_id}_log.txt")
    
    if os.path.exists(structured_job_log):
        log_options.append("📋 Job Execution Log (Structured)")
    elif os.path.exists(compat_job_log):
        log_options.append("📋 Job Execution Log (Compatibility)")
    else:
        # Check if comprehensive logs are available in GCS
        try:
            import subprocess
            gcs_log_path = f"gs://rapid_modeler_app/automl_jobs/{job_id}/logs/job_execution.log"
            check_cmd = ["gsutil", "ls", gcs_log_path]
            result = subprocess.run(check_cmd, capture_output=True, text=True, timeout=10)
            if result.returncode == 0:
                log_options.append("📋 Job Execution Log (GCS)")
        except Exception:
            pass
    ...
    if selected_log in ["🔧 Execution Script (Organized)", "🔧 Execution Script (Compatibility)"]:
        st.subheader(selected_log)
        
        # Determine which script file to use
        if selected_log == "🔧 Execution Script (Organized)":
            script_file_path = organized_script
            file_name = f"{job_id}_script.py"
        else:
            script_file_path = compat_script
            file_name = f"{job_id}_script.py"
        ...
        try:
            ...
            with col2:
                st.info(f"📄 File: {file_name} | 📏 Lines: {len(script_content.split(chr(10)))}")
    ...
    elif selected_log in ["📋 Job Execution Log (Structured)", "📋 Job Execution Log (Compatibility)", "📋 Job Execution Log (GCS)"]:
        st.subheader(selected_log)
        
        # Determine which log file to use
        if selected_log == "📋 Job Execution Log (Structured)":
            log_file_path = structured_job_log
            file_name = f"{job_id}_job_execution.log"
        elif selected_log == "📋 Job Execution Log (GCS)":
            # Download from GCS
            try:
                import subprocess
                import tempfile
                ...
                st.info("📥 Downloading comprehensive logs from GCS...")
                download_cmd = ["gsutil", "cp", gcs_log_path, log_file_path]
                result = subprocess.run(download_cmd, capture_output=True, text=True, timeout=60)
                
                if result.returncode != 0:
                    st.error(f"Failed to download logs from GCS: {result.stderr}")
                    return
                    
                file_name = f"{job_id}_comprehensive_execution.log"
                
            except Exception as e:
                st.error(f"Error downloading logs from GCS: {e}")
                return
        else:
            log_file_path = compat_job_log
        ...
        try:
            ...
            with col2:
                st.info(f"📄 File: {file_name} | 📏 Lines: {len(log_content.split(chr(10)))}")
            
            st.code(log_content, language='text')
            
            # Clean up temporary file if it was downloaded from GCS
            if selected_log == "📋 Job Execution Log (GCS)" and os.path.exists(log_file_path):
                try:
            ...
            if selected_log == "📋 Job Execution Log (GCS)" and 'log_file_path' in locals() and os.path.exists(log_file_path):
                try:
                    os.unlink(log_file_path)
                except Exception:
                    pass
...
def display_feature_profiling(output_dir: str, job_id: str):
    """
    Display comprehensive feature profiling results with PSI analysis.
    
    Args:
        output_dir: Directory containing job results
        job_id: Job identifier
    """
    import pandas as pd
    import numpy as np
    
    st.header("📊 Feature Profiling & Population Stability Index")
    st.markdown("*Comprehensive analysis of feature distributions and stability across datasets*")
    
    # Look for feature profiling JSON files (only in direct output directory)
    profiling_files = []
    for pattern in ['*feature_profiling*.json', '*profiling*.json']:
        profiling_files.extend(glob.glob(os.path.join(output_dir, pattern)))
    
    # Remove duplicates and sort
    profiling_files = sorted(list(set(profiling_files)))
    
    if not profiling_files:
        st.warning("🔍 No feature profiling results found")
        st.info("💡 Feature profiling results are generated during the AutoML pipeline and saved as JSON files")
        return
    
    # Select profiling file if multiple exist
    if len(profiling_files) > 1:
        st.subheader("📁 Multiple Profiling Files Found")
        selected_file = st.selectbox(
            "Select profiling file:",
            profiling_files,
            format_func=lambda x: os.path.basename(x)
        )
    else:
        selected_file = profiling_files[0]
        st.info(f"📄 Using: `{os.path.basename(selected_file)}`")
    ...
    # Check if this is a multi-dataset file (has 'train', 'oot1', etc.) or single dataset
    if 'train' in profiling_data or any(key in profiling_data for key in ['oot1', 'oot2']):
        # Multi-dataset file - use 'train' data by default
        dataset_key = 'train' if 'train' in profiling_data else list(profiling_data.keys())[0]
        dataset_data = profiling_data[dataset_key]
        
        # Show dataset selector if multiple datasets
        available_datasets = [k for k in profiling_data.keys() if k in ['train', 'oot1', 'oot2']]
        if len(available_datasets) > 1:
            selected_dataset = st.selectbox(
                "Select Dataset:",
                available_datasets,
                index=0,
                key=f"dataset_selector_{selected_file}"
            )
            dataset_data = profiling_data[selected_dataset]
        
        # Extract sections from selected dataset
        univariate_data = dataset_data.get('univariate_analysis', {})
        bivariate_data = dataset_data.get('bivariate_analysis', {})
        correlation_data = dataset_data.get('correlation_matrix', {})
        psi_data = dataset_data.get('psi_analysis', dataset_data.get('stability_summary', {}))
        stability_summary = dataset_data.get('stability_summary', {})
        selected_features = dataset_data.get('selected_features', [])
    else:
        # Single dataset file
        univariate_data = profiling_data.get('univariate_analysis', {})
        bivariate_data = profiling_data.get('bivariate_analysis', {})
        correlation_data = profiling_data.get('correlation_matrix', {})
        psi_data = profiling_data.get('psi_analysis', profiling_data.get('stability_summary', {}))
        stability_summary = profiling_data.get('stability_summary', {})
        selected_features = profiling_data.get('selected_features', [])
    ...
    with col1:
        # Features are directly under univariate_data, not under 'features'
        total_features = len([k for k in univariate_data.keys() if k not in ['stability_summary']])
        st.metric("🎯 Features Analyzed", total_features)
    
    with col2:
        overall_grade = stability_summary.get('overall_stability_grade', 'N/A')
        overall_score = stability_summary.get('overall_stability_score', 0)
        st.metric("📊 Stability Grade", f"{overall_grade} ({overall_score:.1f})")
    
    with col3:
        stability_warnings = len(stability_summary.get('quality_issues', []))
        st.metric("⚠️ Stability Warnings", stability_warnings)
    
    with col4:
        if psi_data and 'dataset_stability_summary' in psi_data:
            avg_psi = psi_data['dataset_stability_summary'].get('average_psi', 0)
            st.metric("📈 Average PSI", f"{avg_psi:.4f}")
        else:
            st.metric("📈 PSI Status", "Baseline Only")
    
    # === PSI ANALYSIS SECTION ===
    if psi_data:
        st.subheader("🎯 Population Stability Index (PSI) Analysis")
        
        # Use the stability_summary data we have
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("**📊 PSI Summary**")
            st.write(f"• **Overall Stability Score**: {psi_data.get('overall_stability_score', 0):.1f}/100")
            st.write(f"• **Overall Stability Grade**: {psi_data.get('overall_stability_grade', 'N/A')}")
            st.write(f"• **Features Analyzed**: {psi_data.get('features_analyzed', 0)}")
            
        with col2:
            st.markdown("**🎯 Data Quality**")
            quality_issues = psi_data.get('quality_issues', [])
            if quality_issues:
                st.write(f"• **Quality Issues**: {len(quality_issues)}")
                for issue in quality_issues[:3]:  # Show first 3 issues
                    st.write(f"  - {issue}")
            else:
                st.write("• **Quality Issues**: None detected")
            st.write(f"• **Total Rows**: {psi_data.get('total_rows', 0):,}")
        ...
        with st.expander("📖 PSI Interpretation Guide"):
            ...
            **Population Stability Index (PSI) Thresholds:**
            ...
            - **0.10 - 0.20**: 🟡 **Moderately Stable** - Minor population shift
            - **0.20 - 0.25**: 🟠 **Unstable** - Moderate population shift, investigate
            - **> 0.25**: 🔴 **Very Unstable** - Significant population shift, model may need retraining
            """)
        
        # Detailed PSI by Feature
        # Check for PSI data in different possible locations
        psi_features = {}
        if 'overall_stability_assessment' in psi_data:
            psi_features = psi_data['overall_stability_assessment']
        elif 'stability_summary' in psi_data and 'feature_quality_summary' in psi_data['stability_summary']:
            psi_features = psi_data['stability_summary']['feature_quality_summary']
        elif 'feature_quality_summary' in psi_data:
            psi_features = psi_data['feature_quality_summary']
        
        if psi_features:
            st.markdown("**📋 PSI by Feature**")
            
            psi_records = []
            for feature, psi_info in psi_features.items():
                # Handle both old and new PSI data structures
                psi_scores = psi_info.get('psi_scores', {})
                single_psi = psi_info.get('psi_score', 0.0)
                max_psi = max(psi_scores.values()) if psi_scores else single_psi
                
                # Get stability info
                stability_status = psi_info.get('stability_status', 'Unknown')
                distribution_shift = psi_info.get('distribution_shift', 'None')
                stability_grade = psi_info.get('preliminary_stability_grade', 'N/A')
                
                psi_records.append({
                    'Feature': feature,
                    'PSI Score': f"{max_psi:.4f}",
                    'Stability Status': stability_status,
                    'Stability Grade': stability_grade,
                    'Distribution Shift': distribution_shift,
                    'Train vs OOT1': f"{psi_scores.get('train_vs_oot1', single_psi):.4f}" if psi_scores or single_psi > 0 else 'N/A',
                    'Train vs OOT2': f"{psi_scores.get('train_vs_oot2', single_psi):.4f}" if psi_scores or single_psi > 0 else 'N/A',
                    'OOT1 vs OOT2': f"{psi_scores.get('oot1_vs_oot2', 0):.4f}" if 'oot1_vs_oot2' in psi_scores else 'N/A'
                })
            
            if psi_records:
                psi_df = pd.DataFrame(psi_records)
                
                # Check if all PSI scores are 0 (baseline)
                all_baseline = all(record['Stability Status'] == 'baseline' for record in psi_records)
                if all_baseline:
                    st.info("ℹ️ **All features show baseline stability (PSI = 0.0)**. This indicates that the training and OOT datasets have identical distributions, which is expected when using the same data source for all datasets.")
                
                # Color-code by stability status
                def highlight_stability(row):
                    if row['Stability Status'] == 'baseline':
                        return ['background-color: #e3f2fd'] * len(row)  # Light blue for baseline
                    elif row['Stability Status'] == 'stable':
                        return ['background-color: #d4edda'] * len(row)
                    elif row['Stability Status'] == 'moderately_stable':
                        return ['background-color: #fff3cd'] * len(row)
                    elif row['Stability Status'] == 'unstable':
                        return ['background-color: #f8d7da'] * len(row)
                    elif row['Stability Status'] == 'very_unstable':
                        return ['background-color: #f5c6cb'] * len(row)
                    else:
                        return [''] * len(row)
                
                st.dataframe(
                    psi_df.style.apply(highlight_stability, axis=1),
                    use_container_width=True
                )
    ...
    # Features are directly under univariate_data, not under 'features'
    features_data = {k: v for k, v in univariate_data.items() if k not in ['stability_summary']}
    
    # Check if unique counts are missing
    has_unique_counts = any(stats.get('unique_count', 0) > 0 for stats in features_data.values())
    if not has_unique_counts and features_data:
        st.info("ℹ️ **Note**: Unique counts show as 'N/A' because cardinality calculation was not performed during feature profiling. This is a known issue that will be addressed in future runs.")
    ...
    if features_data:
        ...
        for feature, stats in features_data.items():
            psi_assessment = stats.get('psi_assessment', {})
            
            record = {
                'Feature': feature,
                'Type': stats.get('feature_type', 'Unknown'),
                'Data Type': stats.get('data_type', 'Unknown'),
                'Missing %': f"{stats.get('missing_percentage', 0):.1f}%",
                'Unique Count': stats.get('unique_count', 'N/A') if stats.get('unique_count', 0) == 0 else stats.get('unique_count', 0),
                'Stability Grade': psi_assessment.get('preliminary_stability_grade', 'N/A')
            }
        ...
        if summary_records:
            ...
            with col1:
                feature_type_filter = st.multiselect(
                    ...
                    options=summary_df['Type'].unique(),
                    default=summary_df['Type'].unique()
                )
            
            with col2:
                stability_filter = st.multiselect(
                    "Filter by Stability Grade:",
                    options=summary_df['Stability Grade'].unique(),
                    default=summary_df['Stability Grade'].unique()
                )
            
            # Apply filters
            filtered_df = summary_df[
                (summary_df['Type'].isin(feature_type_filter)) &
                (summary_df['Stability Grade'].isin(stability_filter))
            ]
            
            # Color-code by stability grade
            ...
            st.dataframe(
                filtered_df.style.apply(highlight_grade, axis=1),
                use_container_width=True
            )
    
    # === STABILITY WARNINGS SECTION ===
    if stability_summary.get('quality_issues'):
        st.subheader("⚠️ Stability Warnings")
        
        warnings_data = []
        for issue in stability_summary['quality_issues']:
            warnings_data.append({
                'Feature': issue.get('feature', 'Unknown'),
                'Warning Type': issue.get('warning', issue.get('issue', 'Unknown')),
                'Severity': issue.get('severity', 'Unknown'),
                'Description': issue.get('description', 'No description'),
                'Value': issue.get('value', issue.get('percentage', 'N/A'))
            })
        ...
        if warnings_data:
            ...
            st.dataframe(
                warnings_df.style.apply(highlight_severity, axis=1),
                use_container_width=True
            )
    ...
    if bivariate_data:
        ...
        # Select features to display - show all features that ended up in the final model
        available_features = list(bivariate_data.keys())
        
        # Filter to only show features that are in the selected_features (final model features)
        if selected_features:
            model_features = [f for f in available_features if f in selected_features]
            if model_features:
                sorted_features = model_features
                st.info(f"📊 Showing bivariate analysis for all {len(model_features)} features selected for the final model.")
            else:
                # Fallback if no overlap found
                sorted_features = available_features
                st.warning(f"⚠️ No overlap found between bivariate features and selected model features. Showing all {len(available_features)} available features.")
        else:
            # Fallback if no selected_features available
            sorted_features = available_features
            st.info(f"📊 Showing bivariate analysis for all {len(available_features)} available features.")
        
        # Display bivariate analysis for selected features
        for feature in sorted_features:
            feature_analysis = bivariate_data[feature]
            
            with st.expander(f"📊 {feature} vs Target Analysis", expanded=False):
                analysis_type = feature_analysis.get('analysis_type', 'unknown')
                
                if analysis_type == 'target_distribution_by_bins':
                    # Numerical feature - show distribution by bins
                    distributions = feature_analysis.get('distributions', {})
                    bin_labels = feature_analysis.get('bin_labels', {})
                    
                    if distributions:
                        # Prepare data for visualization
                        bin_data = []
                        for bin_key, bin_info in distributions.items():
                            bin_label = bin_labels.get(bin_key, bin_key)
                            target_dist = bin_info.get('target_distribution', {})
                            total_count = bin_info.get('total_count', 0)
                            
                            for target_val, target_info in target_dist.items():
                                bin_data.append({
                                    'Bin': bin_label,
                                    'Target': f"Class {target_val}",
                                    'Count': target_info.get('count', 0),
                                    'Percentage': target_info.get('percentage', 0),
                                    'Total_in_Bin': total_count
                                })
                        
                        if bin_data:
                            bin_df = pd.DataFrame(bin_data)
                            
                            # Sort bins by their values for proper ordering
                            def sort_bin_key(bin_name):
                                try:
                                    # Try to extract numeric values from bin labels like "0.0-100.0" or "(-inf, 100]" or "[1.00, 2.00]"
                                    import re
                                    numbers = re.findall(r'-?\d+\.?\d*', str(bin_name))
                                    if len(numbers) >= 2:
                                        # Use both start and end values for sorting: (start_value, end_value)
                                        return (float(numbers[0]), float(numbers[1]))
                                    elif len(numbers) == 1:
                                        # Use single number twice for sorting
                                        return (float(numbers[0]), float(numbers[0]))
                                    else:
                                        return (float('inf'), float('inf'))  # Put non-numeric bins at the end
                                except:
                                    return (float('inf'), float('inf'))
                            
                            # Get unique bins and sort them
                            unique_bins = sorted(bin_df['Bin'].unique(), key=sort_bin_key)
                            
                            # Reorder the DataFrame based on sorted bins
                            bin_df['Bin'] = pd.Categorical(bin_df['Bin'], categories=unique_bins, ordered=True)
                            bin_df = bin_df.sort_values('Bin')
                            ...
                            fig = px.bar(
                                ...
                                y='Percentage',
                                color='Target',
                                title=f"{feature} - Target Distribution by Bins",
                                labels={'Percentage': 'Percentage (%)', 'Bin': f'{feature} Bins'},
                                text='Percentage'
                            )
                ...
                elif analysis_type == 'target_distribution_by_category':
                    # Categorical feature - show distribution by categories
                    distributions = feature_analysis.get('distributions', {})
                    
                    if distributions:
                        # Prepare data for visualization
                        cat_data = []
                        for cat_val, cat_info in distributions.items():
                            if cat_val == 'OTHER_CATEGORIES':
                                cat_label = f"Others ({cat_info.get('category_count', 0)} categories)"
                            else:
                                cat_label = str(cat_val)
                            
                            target_dist = cat_info.get('target_distribution', {})
                            total_count = cat_info.get('total_count', 0)
                        ...
                        if cat_data:
                            ...
                            unique_categories = sorted(cat_df['Category'].unique())
                            cat_df['Category'] = pd.Categorical(cat_df['Category'], categories=unique_categories, ordered=True)
                            cat_df = cat_df.sort_values('Category')
                            ...
                            fig = px.bar(
                                ...
                                y='Percentage',
                                color='Target',
                                title=f"{feature} - Target Distribution by Categories",
                                labels={'Percentage': 'Percentage (%)', 'Category': f'{feature} Categories'},
                                text='Percentage'
                            )
                            fig.update_traces(texttemplate='%{text:.1f}%', textposition='inside')
                            fig.update_layout(xaxis_tickangle=-45)
    ...
    if correlation_data and 'correlation_matrix' in correlation_data:
        ...
        target_correlations = correlation_data.get('target_correlations', {})
        ...
        if target_correlations:
            ...
            features = [item[0] for item in sorted_corrs[:20]]  # Top 20
            correlations = [item[1] for item in sorted_corrs[:20]]
            ...
            fig = go.Figure(data=go.Bar(
                ...
                marker=dict(
                    color=correlations,
                    colorscale='RdBu_r',  # Consistent RdBu palette (reversed)
                    cmin=-0.5,  # Adjusted range for better color saturation
                    cmax=0.5,   # This makes colors more vibrant
                    colorbar=dict(title="Correlation"),
                    line=dict(color='white', width=0.5)  # Add white borders for clarity
                ),
                text=[f'{corr:.3f}' for corr in correlations],  # Show correlation values
                textposition='auto'
            ))
        ...
        if isinstance(corr_matrix, dict) and len(corr_matrix) > 1:
            st.markdown("**🔗 Feature-to-Feature Correlation Matrix**")
            
            # Convert correlation matrix dict to DataFrame for heatmap
            # Get all features (excluding target if present)
            target_col = correlation_data.get('target_column', 'deposit')
            features = [f for f in corr_matrix.keys() if f != target_col]
            ...
            for feature1 in features:
                for feature2 in features:
                    if feature1 in corr_matrix and feature2 in corr_matrix[feature1]:
                        corr_df.loc[feature1, feature2] = corr_matrix[feature1][feature2]
                    elif feature2 in corr_matrix and feature1 in corr_matrix[feature2]:
                        corr_df.loc[feature1, feature2] = corr_matrix[feature2][feature1]
                    elif feature1 == feature2:
                        corr_df.loc[feature1, feature2] = 1.0
                    else:
                        corr_df.loc[feature1, feature2] = 0.0
            ...
            fig_heatmap.update_layout(
                title="Feature-to-Feature Correlation Matrix",
            ...
            for i, feature1 in enumerate(features):
                for j, feature2 in enumerate(features):
                    if i < j:  # Avoid duplicates and self-correlation
                        corr_val = corr_df.loc[feature1, feature2]
                        if abs(corr_val) > 0.7:  # High correlation threshold
                            high_corr_pairs.append((feature1, feature2, corr_val))
            
            if high_corr_pairs:
                st.markdown("**⚠️ High Correlation Pairs (|r| > 0.7)**")
                ...
                if len(high_corr_pairs) > 0:
                    st.info(f"💡 Found {len(high_corr_pairs)} highly correlated feature pairs. Consider feature selection to reduce multicollinearity.")
            else:
                st.success("✅ No highly correlated feature pairs found (|r| > 0.7)")
    
    # === RAW DATA SECTION ===
    with st.expander("🔍 Raw Profiling Data"):
