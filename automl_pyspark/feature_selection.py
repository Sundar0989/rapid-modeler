...
def random_forest_feature_selection(
    ...
    for col in sampled_df.columns:  # Use sampled_df instead of df
        if col != target_column:
            ...
            try:
                ...
                col_type = sampled_df.schema[col].dataType.typeName()
                if col_type in ['double', 'float', 'integer', 'long', 'int', 'bigint', 'decimal']:
    ...
    # Save to Excel (with fallback to CSV if openpyxl not available)
    ...
    try:
        ...
        print(f"✅ Feature importance saved to Excel: {excel_path}")
    except ImportError as e:
        if 'openpyxl' in str(e):
            print(f"⚠️ Excel export failed: {e}")
            print(f"🔄 Falling back to CSV export...")
            csv_path = os.path.join(output_dir, f'feature_importance_{user_id}_{model_literal}.csv')
            importance_df.to_csv(csv_path, index=False)
            print(f"✅ Feature importance saved to CSV: {csv_path}")
            excel_path = csv_path  # Update path for return value
        else:
            raise
    except Exception as e:
        ...
        try:
            fallback_path = f'feature_importance_{user_id}_{model_literal}.csv'
            importance_df.to_csv(fallback_path, index=False)
            print(f"✅ Feature importance saved to fallback CSV: {fallback_path}")
...
def save_feature_importance(output_dir, user_id, mdl_ltrl, importance_df):
    ...
    # Save to output directory with cross-platform path (with fallback to CSV)
