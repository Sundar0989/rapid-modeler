...
class ConfigManager:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            possible_paths = [
                str(self.config_path),  # Original path
                os.path.join(os.getcwd(), 'config.yaml'),  # Current directory
            ]
            
            # Add environment-specific paths
            if is_docker:
                possible_paths.extend([
                    os.path.join('/app', 'automl_jobs', 'config.yaml'),  # Docker: /app/automl_jobs directory
                    os.path.join('/app', 'automl_results', 'config.yaml'),  # Docker: /app/automl_results directory
                ])
            elif is_dataproc:
                # Dataproc Serverless: use current working directory and temp directories
                possible_paths.extend([
                    os.path.join(os.getcwd(), 'automl_jobs', 'config.yaml'),  # Dataproc: automl_jobs directory
                    os.path.join(os.getcwd(), 'automl_results', 'config.yaml'),  # Dataproc: automl_results directory
                    os.path.join('/tmp', 'automl_jobs', 'config.yaml'),  # Dataproc: temp directory
                    os.path.join('/tmp', 'automl_results', 'config.yaml'),  # Dataproc: temp directory
                ])
            else:
                # Local development: use current directory
                possible_paths.extend([
                    os.path.join(os.getcwd(), 'automl_jobs', 'config.yaml'),  # Local: automl_jobs directory
                    os.path.join(os.getcwd(), 'automl_results', 'config.yaml'),  # Local: automl_results directory
                ])
            
            # Always try package directory as fallback
            possible_paths.append(os.path.join(os.path.dirname(__file__), 'config.yaml'))  # Package directory
            ...
            print(f"⚠️ Configuration file not found in any expected locations. Using default configuration.")
    ...
    def _detect_package_availability(self):
        ...
        # Check LightGBM availability (native LightGBM)
        try:
            import lightgbm as lgb
            lightgbm_available = True
            print(f"✅ Native LightGBM detected and available (version: {lgb.__version__})")
        except ImportError:
            lightgbm_available = False
            print("⚠️ Native LightGBM not available")
