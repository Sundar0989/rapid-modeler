...
class DataProcessor:
    ...
    def preprocess(self, data: DataFrame, target_column: str, config: Dict) -> tuple:
        ...
        original_vars = feature_vars.copy()
        ...
        # Track variables dropped due to missing values
        if self.variable_tracker:
            dropped_missing = [var for var in original_vars if var not in selected_vars]
            if dropped_missing:
                self.variable_tracker.filter_by_missing_values(dropped_missing, config['missing_threshold'])
        ...
        if self.variable_tracker:
            self.variable_tracker.update_variable_types(categorical_vars, numerical_vars)
        ...
        if categorical_vars_to_drop:
            X_selected = remove_high_cardinality_categoricals(X_selected, categorical_vars_to_drop)
            ...
            # numerical_vars stays unchanged
            print(f"Updated categorical variables: {len(categorical_vars)}")
            print(f"Numerical variables unchanged: {len(numerical_vars)}")
    ...
    def select_features(self, data: DataFrame, target_column: str, max_features: int = 30) -> Tuple[DataFrame, List[str]]:
        """
        ...
        try:
            ...
            size_category = 'medium'
            print(f"📊 Using default dataset category: {size_category}")
        ...
        if row_count > 5_000_000:
            ...
            feature_selection_data.persist(StorageLevel.MEMORY_AND_DISK)
        ...
        if total_features > self.sequential_threshold:
            print(f"🔄 Large feature set detected ({total_features} > {self.sequential_threshold}). Using sequential feature selection...")
            ...
            print(f"📊 Standard feature selection for moderate feature set ({total_features} <= {self.sequential_threshold})...")
        ...
        if hasattr(self, 'variable_tracker') and self.variable_tracker:
            method_name = "sequential_feature_selection" if total_features > self.sequential_threshold else "standard_feature_selection"
    ...
    def split_and_scale(self, data: DataFrame, train_size: float = 0.7, 
        ...
        print(f"🔍 VERIFICATION: data contains {data.count():,} rows after analyze_class_distribution")
        # Split data
        train, valid, test = train_valid_test_split(data, train_size, valid_size, seed)
        
        # Keep a copy of original training data
        train_original = train
        print(f"🔍 VERIFICATION: data contains {data.count():,} rows after train_valid_test_split")
        print(f"🔍 VERIFICATION: train contains {train.count():,} rows after train_valid_test_split")
        print(f"🔍 VERIFICATION: valid contains {valid.count():,} rows after train_valid_test_split")
        print(f"🔍 VERIFICATION: test contains {test.count():,} rows after train_valid_test_split")
    ...
    def apply_preprocessing(self, data: DataFrame, feature_vars: List[str], 
        ...
        # avoid "column does not exist" errors (e.g., SNAPSHOT_DAY_ID does
        # not exist), add any missing raw categorical or numerical columns
        # with null values.  Categorical columns are cast to string so that
        # encoding behaves correctly; numerical columns are cast to double
        # so that imputation can later convert them appropriately.
    ...
    # ========================================================================
    # REMOVED: Duplicate class distribution analysis methods
    # ========================================================================
    # The following methods were removed to eliminate code duplication:
    # - analyze_class_distribution() - now handled by self.data_balancer.analyze_class_distribution()
    # - detect_imbalance() - now handled by self.data_balancer.detect_imbalance()
    #
    # This eliminates ~78 lines of duplicate code and follows single responsibility principle.
    # All class distribution analysis is now centralized in the DataBalancer class.
    # The split_and_scale() method correctly delegates to self.data_balancer methods.
    # ========================================================================
    
    # ========================================================================
    # REMOVED: Duplicate balancing methods - delegated to DataBalancer class
    # ========================================================================
    # The following methods were removed to eliminate code duplication:
    # - upsample_minority_classes() - now handled by self.data_balancer
    # - _oversample_data() - now handled by self.data_balancer
    # - _smote_data() - now handled by self.data_balancer
    # - _generate_smote_samples() - now handled by self.data_balancer
    #
    # This eliminates ~295 lines of duplicate code and follows single responsibility principle.
    # All balancing functionality is now centralized in the DataBalancer class.
    # The split_and_scale() method correctly delegates to self.data_balancer.upsample_minority_classes()
    # ========================================================================
    ...
    def process_data(self, train_data: Union[str, DataFrame], target_column: Optional[str] = None,
        ...
        if target_column:
            ...
            self.variable_tracker = VariableTracker(task_type="classification")
            self.variable_tracker.initialize_variables(train_data.columns, target_column)
        ...
        if target_column:
            ...
            if self.variable_tracker:
                try:
                    ...
                    report_path = f"{output_dir}/classification_variable_tracking_report.xlsx"
            ...
            numeric_cols = [col for col in train_data.columns 
                           ...
                           train_data.schema[col].dataType.typeName() in ['double', 'float', 'integer', 'long', 'decimal']]
