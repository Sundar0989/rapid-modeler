...
class DataBalancer:
    ...
    def analyze_class_distribution(self, data: DataFrame, target_column: str) -> Dict[str, Any]:
        ...
        if sum_of_class_counts != total_count:
            ...
            print(f"💡 This should now be fixed by upstream cache clearing")
        else:
    ...
    def upsample_minority_classes(self, data: DataFrame, target_column: str, 
                                ...
                                max_ratio: float = 0.05, seed: int = 12345, 
        ...
        if method == 'oversample':
            ...
            original_dataset_stats = config.get('original_dataset_stats') if config else None
            return self._smote_data(data, target_column, class_stats, max_ratio, seed, k_neighbors, original_dataset_stats)
        else:
    ...
    def _smote_data(self, data: DataFrame, target_column: str, 
                   class_stats: Dict[str, Any], max_ratio: float, 
                   seed: int, k_neighbors: int = 5, original_dataset_stats: Dict[str, Any] = None) -> DataFrame:
        """
        ...
        # Use original dataset stats if provided, otherwise fall back to current data stats
        if original_dataset_stats:
            # Calculate target based on original full dataset size for proper distribution
            original_total_count = original_dataset_stats['total_count']
            target_minority_count = int(original_total_count * max_ratio)
            print(f"Target minority class size: {target_minority_count} samples ({max_ratio:.1%} of original dataset with {original_total_count:,} total samples)")
        else:
            # Fallback to current behavior for backward compatibility
            majority_count = class_stats['majority_count']
            target_minority_count = int(majority_count * max_ratio)
            print(f"Target minority class size: {target_minority_count} samples ({max_ratio:.1%} of current majority class)")
    ...
    def _generate_smote_samples(self, data: DataFrame, target_column: str, 
        ...
        for row in class_rows:
            # Get feature values from individual columns and convert to float
            # This handles decimal.Decimal types from BigQuery
            feature_values = []
            for col in feature_cols:
                value = row[col]
                # Convert decimal.Decimal and other numeric types to float
        ...
        # Calculate feature distributions for intelligent base sample selection
        # This helps generate samples that better match the target distribution
        feature_means = np.mean(feature_vectors, axis=0)
        feature_stds = np.std(feature_vectors, axis=0)
        
        # Create weighted probabilities based on distance from mean
        # Samples closer to the distribution center get higher probability
        sample_weights = []
        for i, sample in enumerate(feature_vectors):
            # Calculate normalized distance from distribution center
            if len(feature_stds) > 0 and np.any(feature_stds > 0):
                # Use Mahalanobis-like distance (simplified)
                normalized_diff = np.abs(sample - feature_means) / (feature_stds + 1e-8)
                distance_from_center = np.mean(normalized_diff)
                # Convert distance to weight (closer = higher weight)
                weight = 1.0 / (1.0 + distance_from_center)
            else:
                weight = 1.0  # Equal weights if no variation
            sample_weights.append(weight)
        
        # Normalize weights to probabilities
        total_weight = sum(sample_weights)
        if total_weight > 0:
            sample_probabilities = [w / total_weight for w in sample_weights]
        else:
            sample_probabilities = [1.0 / len(feature_vectors)] * len(feature_vectors)
        ...
        for _ in range(num_samples):
            # Select base sample using distribution-aware probabilities
            base_idx = np.random.choice(len(feature_vectors), p=sample_probabilities)
