    ...
    def _save_selection_results(self, df: pd.DataFrame, best_model_info: Dict[str, Any]):
        ...
        try:
            ...
            try:
                ...
                print(f"✅ Model selection results saved to Excel: {output_file}")
            except ImportError as e:
                if 'openpyxl' in str(e):
                    print(f"⚠️ Excel export failed: {e}")
                    print(f"🔄 Falling back to CSV export...")
                    csv_file = os.path.join(self.output_dir, 'model_selection_results.csv')
                    df.to_csv(csv_file, index=False)
                    output_file = csv_file
                    print(f"✅ Model selection results saved to CSV: {output_file}")
                else:
                    raise
