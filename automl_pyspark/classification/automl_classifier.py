...
class AutoMLClassifier:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            config_path: Path to YAML configuration file OR job configuration dictionary
        ...
        if isinstance(output_dir, SparkSession):
            ...
            if config_path and isinstance(config_path, str) and not config_path.endswith(('.yaml', '.yml')):
                output_dir = config_path
                config_path = None
            else:
                output_dir = 'automl_output'
        
        # Handle case where config_path is actually a job configuration dictionary
        job_config_dict = None
        if isinstance(config_path, dict):
            print("🔄 Detected job configuration dictionary - will convert to YAML configuration")
            job_config_dict = config_path
            config_path = None  # Don't pass dict to ConfigManager
        ...
        # Handle job configuration dictionary if provided
        if job_config_dict is not None:
            print("🔄 Applying job configuration dictionary...")
            # Extract key parameters from job_config_dict
            if 'user_id' in job_config_dict:
                self.actual_user_id = job_config_dict['user_id']
            if 'model_id' in job_config_dict:
                self.actual_model_id = job_config_dict['model_id']
            if 'model_name' in job_config_dict:
                self.actual_model_literal = job_config_dict['model_name']
            if 'output_dir' in job_config_dict:
                output_dir = job_config_dict['output_dir']
            
            # Override config with job configuration
            self.config_manager.override_config(job_config_dict)
            print("✅ Job configuration applied successfully")
        ...
        # Set output directory and user_id
        self.output_dir = output_dir
        self.user_id = self.actual_user_id  # For DataInputManager compatibility
    ...
    def _log_available_models(self):
        ...
        # Note: config values should already be converted from "auto" to boolean by config manager
        xgboost_enabled = self.config.get('run_xgboost', False)
        lightgbm_enabled = self.config.get('run_lightgbm', False)
        
        # Handle "auto" values that weren't converted (fallback)
        if xgboost_enabled == "auto":
            xgboost_enabled = XGBOOST_AVAILABLE
        if lightgbm_enabled == "auto":
            lightgbm_enabled = LIGHTGBM_AVAILABLE
            
        advanced_models = [
            ('XGBoost', xgboost_enabled, XGBOOST_AVAILABLE),
            ('LightGBM', lightgbm_enabled, LIGHTGBM_AVAILABLE)
        ]
    ...
    def fit(self, 
        ...
        if isinstance(train_data, str):
            ...
            # CRITICAL FIX: Store original table reference BEFORE data loading (not after)
            # train_data might be modified during loading to point to sample tables
            ...
            # Extract where_clause from bigquery_options if available
            bigquery_options = kwargs.get('bigquery_options', {})
            where_clause = bigquery_options.get('where_clause')
            
            # Load data with intelligent sampling for feature engineering phase only
            train_data, metadata = data_manager.load_data(
                ...
                where_clause=where_clause  # Pass the where_clause
            )
            
            # Store the sampled data for feature engineering
            self._sampled_data_for_feature_engineering = train_data
            
            print(f"📊 Loaded sampled data for feature engineering: {train_data.count():,} rows")
            print(f"💡 Full dataset will be loaded after feature selection for training")
            
            # Store original where_clause and filtered table reference for later use
            self._original_where_clause = where_clause
            if '_filtered_table_reference' in metadata:
                self._filtered_table_reference = metadata['_filtered_table_reference']
    ...
    def _fit_internal(self, train_data: DataFrame, target_column: str, 
        ...
        # Step 2: Feature selection (on sampled data if applicable)
        print("\n2. Feature Selection...")
        ...
        # Step 2.5: Switch to full dataset for model training (if we used sampling)
        print(f"\n🔍 Checking if full dataset switch is needed...")
        print(f"   Has _original_table_reference: {hasattr(self, '_original_table_reference')}")
        print(f"   Has _data_manager: {hasattr(self, '_data_manager')}")
        
        if hasattr(self, '_original_table_reference') and hasattr(self, '_data_manager'):
            print(f"\n2.5. 🔄 Switching to Full Dataset for Model Training...")
            print(f"   📊 Current dataset size: {processed_data.count():,} rows")
            print(f"   🎯 Selected features: {len(top_features)} features")
            
            try:
                # Check if the method exists
                if not hasattr(self._data_manager, 'load_full_data_after_feature_selection'):
                    print(f"   ❌ Method 'load_full_data_after_feature_selection' not found in DataInputManager")
                    print(f"   📊 Continuing with sampled data: {processed_data.count():,} rows")
                else:
                    # Load full data with selected features
                    print(f"   🔄 Loading full dataset with selected features...")
                    print(f"   🔍 DEBUG: _original_table_reference = {self._original_table_reference}")
                    print(f"   🔍 DEBUG: _filtered_table_reference = {getattr(self, '_filtered_table_reference', None)}")
                    ...
                    print(f"   📊 Full dataset loaded: {full_data.count():,} rows")
                    print(f"   📊 Full dataset columns: {full_data.columns}")
                    ...
                    # Reprocess the full data with the same preprocessing steps
                    print(f"   🔄 Reprocessing full dataset with same preprocessing...")
                    ...
                    # Create final dataset with only selected features + target to break lineage and optimize memory
                    # CRITICAL FIX: Convert original feature names to encoded names for processed data selection
                    # top_features contains original names, but full_processed_data has encoded names
                    encoded_feature_names = []
                    for feature in top_features:
                        # Check if encoded version exists in the processed data (use cached columns to prevent re-computation)
                        encoded_name = f"{feature}_encoded"
                        if encoded_name in cached_columns:
                            encoded_feature_names.append(encoded_name)
                        elif feature in cached_columns:
                            encoded_feature_names.append(feature)  # Use original if no encoded version
                        else:
                            print(f"   ⚠️ Feature {feature} not found in processed data (neither original nor encoded)")
                    
                    required_columns = encoded_feature_names + [target_column]
                    print(f"   🔄 Selecting {len(encoded_feature_names)} encoded features + target from processed data")
                    print(f"   📋 Required columns: {required_columns[:5]}{'...' if len(required_columns) > 5 else ''}")
                    
                    # CRITICAL DEBUG: Check row count before and after select operation
                    print(f"   🔍 DEBUG: Verifying full_processed_data integrity...")
                    current_count = full_processed_data.count()
                    print(f"   🔍 DEBUG: full_processed_data before select: {current_count:,} rows")
                    
                    # Additional verification - check if this matches our materialized count
                    if current_count != materialized_count:
                        print(f"   ⚠️ WARNING: Row count mismatch! Expected {materialized_count:,}, got {current_count:,}")
                        print(f"   🔧 Attempting to use the original cached reference...")
                        full_processed_data = full_processed_data_raw
                        current_count = full_processed_data.count()
                        print(f"   🔍 DEBUG: After reference fix: {current_count:,} rows")
                    print(f"   🔍 DEBUG: Checking if all required columns exist in processed data...")
                    available_columns = full_processed_data.columns
                    missing_columns = [col for col in required_columns if col not in available_columns]
                    if missing_columns:
                        print(f"   ❌ MISSING COLUMNS: {missing_columns}")
                        print(f"   📋 Available columns: {available_columns}")
                    else:
                        print(f"   ✅ All {len(required_columns)} required columns found in processed data")
                    
                    full_processed_data_final = full_processed_data.select(required_columns)
                    print(f"   🔍 DEBUG: full_processed_data_final after select: {full_processed_data_final.count():,} rows")
                    print(f"   🔄 Created full_processed_data_final with {len(required_columns)} columns")
                    
                    # Update selected_vars to use encoded names for consistency with processed data
                    self.selected_vars = encoded_feature_names
                    print(f"   🔄 Updated selected_vars to use encoded feature names: {len(self.selected_vars)} features")
                    
                    # Set the final dataset to use for the rest of the pipeline
                    final_dataset = full_processed_data_final
                ...
                print(f"   ❌ Failed to switch to full dataset: {e}")
                print(f"   📊 Continuing with sampled data: {processed_data.count():,} rows")
                import traceback
                traceback.print_exc()
                # Set the final dataset to use for the rest of the pipeline
            ...
            if not hasattr(self, '_original_table_reference'):
                print(f"   💡 Reason: No original table reference (data loaded from file?)")
            if not hasattr(self, '_data_manager'):
                print(f"   💡 Reason: No data manager available")
            print(f"   💡 processed_data remains as sampled data for the rest of the pipeline")
            # Set the final dataset to use for the rest of the pipeline
            final_dataset = processed_data
            # Cache the final dataset since it will be used multiple times in the pipeline
            final_dataset.cache()
        ...
        print(f"🔍 VERIFICATION: final_dataset contains {final_dataset.count():,} rows before split_and_scale")
        
        train_scaled, train_original_scaled, valid_scaled, test_scaled = self._split_and_scale_data(
            final_dataset, target_column
        )
        
        print(f"🔍 VERIFICATION: Train_scaled contains {train_scaled.count():,} rows after split_and_scale")
        print(f"🔍 VERIFICATION: Train_original_scaled contains {train_original_scaled.count():,} rows after split_and_scale")
        print(f"🔍 VERIFICATION: valid_scaled contains {valid_scaled.count():,} rows after split_and_scale")
        print(f"🔍 VERIFICATION: test_scaled contains {test_scaled.count():,} rows after split_and_scale")
        ...
        # Step 4.5: Feature Profiling (for all datasets after preprocessing)
        print("\n4.5. Feature Profiling for All Datasets...")
        self._perform_feature_profiling_all_datasets(
            final_dataset, oot1_scaled, oot2_scaled, 
            top_features, target_column
        )
        ...
        if self.best_model is not None and self.best_model_type is not None:
            try:
                ...
                # OPTIMIZED APPROACH: Use processed data directly to avoid redundant preprocessing
                sample_df = final_dataset  # Use processed data directly
                print(f"   📊 Using processed data for SHAP: {sample_df.count():,} rows")
                
                # For processed data, use the encoded feature names directly
                if self.selected_vars:
                    feature_cols = [col for col in self.selected_vars if col != target_column and col in sample_df.columns]
                    print(f"   🎯 SHAP will analyze {len(feature_cols)} selected features (processed)")
                else:
                    # Fallback to all feature columns except target
                    feature_cols = [col for col in sample_df.columns if col != target_column]
                    print(f"   🎯 SHAP will analyze {len(feature_cols)} available features")
                
                # For processed data, all features are numerical (categorical are encoded)
                categorical_vars_for_shap = []  # No categorical vars since they're encoded
                numerical_vars_for_shap = feature_cols  # All features are numerical after encoding
                print(f"   📊 SHAP feature breakdown: {len(numerical_vars_for_shap)} numerical, {len(categorical_vars_for_shap)} categorical")
                ...
                compute_shap_values(
                    ...
                    model_name=self.best_model_type,
                    max_samples=100,
                )
                print(f"✅ SHAP values computed successfully for best model ({self.best_model_type})")
            except ImportError as e:
                print(f"⚠️ SHAP computation skipped: SHAP library not available ({e})")
                print(f"   💡 Install with: pip install shap>=0.40.0")
            except Exception as e:
    ...
    def _select_features(self, data: DataFrame, target_column: str) -> Tuple[DataFrame, List[str]]:
        """Select the best features for modeling using unified approach."""
        ...
        # Use unified feature selection approach
    ...
    def _split_and_scale_data(self, data: DataFrame, target_column: str) -> Tuple[DataFrame, DataFrame, DataFrame, DataFrame]:
        ...
        print(f"🔍 VERIFICATION: data contains {data.count():,} rows before split_and_scale")
    ...
    def _prepare_oot_datasets(self, oot1_data: Optional[DataFrame], 
        ...
        # Convert encoded column names back to original names for OOT dataset processing
        # This will be reused throughout the method to avoid duplication
        original_column_names = []
        if hasattr(self, 'selected_vars') and self.selected_vars:
        ...
        if oot1_data is not None:
            ...
            try:
                ...
                # (original_column_names already calculated above)
        ...
        # ========================================================================
        # 🚀 OPTIMIZE OOT DATASETS - SELECT ONLY NECESSARY COLUMNS
        # ========================================================================
        # After preprocessing, optimize OOT datasets to only include selected features + target
        # This significantly reduces memory usage and speeds up feature profiling
        
        if hasattr(self, 'selected_vars') and self.selected_vars:
            print(f"\n🚀 Optimizing OOT datasets to use only selected features...")
            
            # Use encoded column names (selected_vars) since OOT datasets are already processed
            # Add target column and features column to the list of required columns
            required_columns = self.selected_vars + [target_column]
            
            # CRITICAL: Include 'features' column for model validation
            # The 'features' column is created by VectorAssembler + StandardScaler pipeline
            # and is required by all trained models for prediction
            if 'features' not in required_columns:
                required_columns.append('features')
            
            # Also include 'assembled_features' if it exists (intermediate step)
            if oot1_scaled is not None and 'assembled_features' in oot1_scaled.columns:
                if 'assembled_features' not in required_columns:
                    required_columns.append('assembled_features')
            elif oot2_scaled is not None and 'assembled_features' in oot2_scaled.columns:
                if 'assembled_features' not in required_columns:
                    required_columns.append('assembled_features')
            
            print(f"📋 Required columns for OOT datasets: {len(required_columns)} columns")
            print(f"   Features: {len(self.selected_vars)}, Target: 1, Vector columns: {len(required_columns) - len(self.selected_vars) - 1}")
            
            # Filter OOT1 dataset to only include required columns
            # (Skip column availability check since it was already done during preprocessing)
            if oot1_scaled is not None:
                original_oot1_columns = len(oot1_scaled.columns)
                oot1_scaled = oot1_scaled.select(required_columns)
                print(f"🔄 OOT1 optimized: {original_oot1_columns} → {len(required_columns)} columns ({original_oot1_columns - len(required_columns)} columns removed)")
            
            # Filter OOT2 dataset to only include required columns
            # (Skip column availability check since it was already done during preprocessing)
            if oot2_scaled is not None:
                original_oot2_columns = len(oot2_scaled.columns)
                oot2_scaled = oot2_scaled.select(required_columns)
                print(f"🔄 OOT2 optimized: {original_oot2_columns} → {len(required_columns)} columns ({original_oot2_columns - len(required_columns)} columns removed)")
        else:
            print("⚠️ No selected features available - skipping OOT dataset optimization")
        
        # Cache datasets only if they exist (not None)
        oot1_cached = oot1_scaled.cache() if oot1_scaled is not None else None
        oot2_cached = oot2_scaled.cache() if oot2_scaled is not None else None
        
        return oot1_cached, oot2_cached
    
    def _perform_feature_profiling_all_datasets(self, train_data: DataFrame,
                                              oot1_data: Optional[DataFrame],
                                              oot2_data: Optional[DataFrame],
                                              selected_features: List[str],
                                              target_column: str):
        """
        Perform feature profiling on all available datasets (train, oot1, oot2).
        
        Args:
            train_data: Preprocessed training data
            oot1_data: Preprocessed OOT1 data (optional)
            oot2_data: Preprocessed OOT2 data (optional)
            selected_features: List of selected feature names
            target_column: Name of target variable
        """
        try:
            from feature_profiling import FeatureProfiler
            
            # Create feature profiler
            profiler = FeatureProfiler(self.spark)
            
            # Use existing cardinality stats from data processor (already computed)
            existing_stats = None
            if hasattr(self.data_processor, 'cardinality_stats'):
                existing_stats = {'cardinality_analysis': self.data_processor.cardinality_stats}
                print(f"♻️ Reusing cardinality stats for {len(self.data_processor.cardinality_stats)} categorical features")
            ...
            if oot1_data is not None:
                datasets_to_profile.append(("oot1", oot1_data))
                print(f"📊 OOT1 dataset available for profiling: {oot1_data.count()} rows")
            
            if oot2_data is not None:
                datasets_to_profile.append(("oot2", oot2_data))
                print(f"📊 OOT2 dataset available for profiling: {oot2_data.count()} rows")
            
            print(f"🔍 Profiling {len(datasets_to_profile)} datasets with {len(selected_features)} selected features")
            
            # Profile each dataset
            all_profiling_results = {}
            
            for dataset_name, dataset in datasets_to_profile:
                print(f"\n📊 Profiling {dataset_name} dataset...")
                
                try:
                    # For training data, use existing stats. For OOT datasets, calculate fresh stats
                    if dataset_name == "train":
                        # Use pre-computed cardinality stats for training data
                        dataset_existing_stats = existing_stats
                        print(f"♻️ Using existing cardinality stats for {dataset_name} dataset")
                    else:
                        # Calculate fresh cardinality stats for OOT datasets
                        print(f"🔄 Calculating fresh cardinality stats for {dataset_name} dataset...")
                        oot_cardinality_stats = self._calculate_cardinality_stats_for_oot(
                            dataset, self.categorical_vars
                        )
                        dataset_existing_stats = {'cardinality_analysis': oot_cardinality_stats}
                        print(f"✅ Calculated cardinality stats for {len(oot_cardinality_stats)} categorical features in {dataset_name}")
                    
                    # Map encoded feature names back to raw names for feature profiling
                    raw_feature_names = []
                    for feature in selected_features:
                        if feature.endswith('_encoded'):
                            # Remove _encoded suffix to get original name
                            raw_name = feature[:-8]  # Remove '_encoded'
                            if raw_name in dataset.columns:
                    ...
                    print(f"🔄 Mapped {len(selected_features)} encoded features to {len(raw_feature_names)} raw features for profiling")
                    if selected_features and raw_feature_names:
                        print(f"   Example: {selected_features[0]} → {raw_feature_names[0]}")
                    
                    profiling_results = profiler.profile_features(
                        data=dataset,
                        selected_features=raw_feature_names,
                        target_column=target_column,
                        task_type="classification",
                        existing_stats=dataset_existing_stats,
                        categorical_vars=self.categorical_vars,
                        numerical_vars=self.numerical_vars
                    )
                    
                    # Add dataset metadata
                    profiling_results['dataset_name'] = dataset_name
                    profiling_results['dataset_size'] = dataset.count()
                    
                    all_profiling_results[dataset_name] = profiling_results
                    
                    # Save individual dataset profiling results
                    import json
                    profiling_file = os.path.join(self.output_dir, f"{self.user_id}_{dataset_name}_feature_profiling.json")
                    with open(profiling_file, 'w') as f:
                        json.dump(profiling_results, f, indent=2, default=str)
                    
                    print(f"✅ {dataset_name} profiling completed and saved to: {profiling_file}")
                    
                except Exception as e:
                    print(f"⚠️ Feature profiling failed for {dataset_name}: {str(e)}")
                    continue
            
            # Save combined profiling results
            if all_profiling_results:
                combined_file = os.path.join(self.output_dir, f"{self.user_id}_all_datasets_feature_profiling.json")
                with open(combined_file, 'w') as f:
                    json.dump(all_profiling_results, f, indent=2, default=str)
                
                print(f"✅ Combined profiling results saved to: {combined_file}")
                print(f"🔍 Successfully profiled {len(all_profiling_results)} datasets with {len(selected_features)} features")
            
        except Exception as e:
            print(f"⚠️ Feature profiling failed: {str(e)}")
            print("Continuing with model training...")
    
    def _calculate_cardinality_stats_for_oot(self, data: DataFrame, categorical_vars: List[str]) -> Dict[str, Any]:
        """
        Calculate cardinality and missing value statistics for OOT datasets.
        
        Args:
            data: OOT DataFrame to analyze
        ...
        for cat_var in categorical_vars:
            if cat_var in data.columns:
                try:
                    # Calculate statistics for this categorical variable
                    total_count = data.count()
                    distinct_count = data.select(cat_var).distinct().count()
                    null_count = data.filter(data[cat_var].isNull()).count()
                    missing_percentage = (null_count / total_count) * 100 if total_count > 0 else 0
                    
                    cardinality_stats[cat_var] = {
                        'distinct_count': distinct_count,
                        'unique_count': distinct_count,  # Same as distinct_count
                        'null_count': null_count,
                        'missing_count': null_count,  # Same as null_count
                        'total_count': total_count,
                        'missing_percentage': missing_percentage
                    }
                    
                except Exception as e:
                    print(f"⚠️ Error calculating stats for {cat_var}: {str(e)}")
    ...
    def _update_config_from_legacy_params(self, user_id: Optional[str], model_id: Optional[str], 
        ...
        legacy_overrides['classification'] = {
            'models': {
                ...
                'run_neural_network': True,
                'run_decision_tree': True,
                'run_xgboost': 'auto',  # Auto-detect advanced models
                'run_lightgbm': 'auto'
            },
            'hyperparameter_tuning': {
