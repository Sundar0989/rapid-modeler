...
try:
    ...
    print(f"✅ Native LightGBM integration available and functional (version: {lgb.__version__})")
except ImportError as e:
...
class NativeLightGBMModel:
    """
    Wrapper for native LightGBM model to provide Spark-compatible interface.
    This class mimics the behavior of Spark ML models.
    """
    
    def __init__(self, lgb_model, feature_names: List[str], label_col: str, 
                 features_col: str, num_classes: int, class_labels: Optional[List] = None):
        """
        Initialize the wrapper with a trained LightGBM model.
        ...
        Args:
            ...
            class_labels: Optional list of class labels for mapping
        """
    ...
    def transform(self, dataset: DataFrame, *args, **kwargs) -> DataFrame:
        """
        Transform a DataFrame using the trained LightGBM model.
        ...
        if self.features_col in pandas_df.columns:
            ...
            # Use individual feature columns
            features_array = pandas_df[self.feature_names].values
        
        # Make predictions
        if self.num_classes > 2:
            # Multi-class classification
            pred_proba = self.lgb_model.predict(features_array)
            predictions = np.argmax(pred_proba, axis=1)
            
            # Create probability vectors
            prob_vectors = [Vectors.dense(prob.tolist()) for prob in pred_proba]
        else:
            # Binary classification
            pred_proba = self.lgb_model.predict(features_array)
            predictions = (pred_proba > 0.5).astype(int)
            
            # Create probability vectors for binary classification
            prob_vectors = [
                Vectors.dense([1.0 - prob, prob]) for prob in pred_proba
            ]
        
        # Add predictions to pandas DataFrame
        pandas_df['prediction'] = predictions.astype(float)
        pandas_df['probability'] = prob_vectors
        
        # Add rawPrediction column for Spark ML compatibility
        if self.num_classes > 2:
            # Multi-class: rawPrediction is the same as probability
            pandas_df['rawPrediction'] = prob_vectors
        else:
            # Binary classification: rawPrediction should be log-odds or raw scores
            # For compatibility, we'll use the probability scores as raw prediction
            raw_predictions = [
                Vectors.dense([np.log(1.0 - prob + 1e-15), np.log(prob + 1e-15)]) for prob in pred_proba
            ]
            pandas_df['rawPrediction'] = raw_predictions
        
        # Convert back to Spark DataFrame
        # Define schema for the result
        original_schema = dataset.schema
        
        # Add prediction, probability, and rawPrediction columns to schema
        result_fields = list(original_schema.fields)
        result_fields.append(StructField("prediction", DoubleType(), True))
        result_fields.append(StructField("probability", VectorUDT(), True))
        result_fields.append(StructField("rawPrediction", VectorUDT(), True))
    ...
    def write(self):
        ...
        Returns a writer object that can save the model.
        """
        return NativeLightGBMWriter(self)
    
    def save(self, path: str):
        """
        Save the LightGBM model to the specified path.
...
class NativeLightGBMWriter:
    """
    Writer class for NativeLightGBMModel to provide Spark ML compatibility.
    """
    
    def __init__(self, model: NativeLightGBMModel):
        self.model = model
...
class NativeLightGBMClassifier:
    """
    Native LightGBM Classifier that provides Spark ML compatible interface.
    """
    
    def __init__(self, featuresCol: str = "features", labelCol: str = "label",
                 objective: str = "binary", numLeaves: int = 31, 
                 numIterations: int = 100, learningRate: float = 0.1,
                 featureFraction: float = 1.0, baggingFraction: float = 1.0,
                 lambdaL1: float = 0.0, lambdaL2: float = 0.0, seed: int = 42):
        """
        Initialize Native LightGBM Classifier.
        
        Args:
            featuresCol: Name of the features column
            labelCol: Name of the label column
            objective: LightGBM objective ('binary', 'multiclass')
    ...
    def fit(self, dataset: DataFrame, *args, **kwargs) -> NativeLightGBMModel:
        """
        Fit the LightGBM model on the training dataset.
        
        Args:
            dataset: Training DataFrame
            
        Returns:
            Trained NativeLightGBMModel
        """
        print(f"🚀 Training Native LightGBM with objective: {self.objective}")
        ...
        # Train model
        print(f"🏋️ Training LightGBM model...")
        lgb_model = lgb.train(
            params,
            train_data,
        ...
        print(f"✅ Native LightGBM model trained successfully!")
        print(f"📊 Model info: {lgb_model.num_trees()} trees, {lgb_model.num_feature()} features")
        
        # Create wrapper model
        wrapper_model = NativeLightGBMModel(
            lgb_model=lgb_model,
...
def lightgbm_model(train, x, y, num_classes=2):
    """Build LightGBM model using Native LightGBM integration."""
    if not LIGHTGBM_AVAILABLE:
        raise ImportError("Native LightGBM not available. Install with: pip install lightgbm>=4.0.0")
    
    print(f"🚀 Building Native LightGBM model for {num_classes} classes")
    
    # Configure LightGBM parameters
    objective = "multiclass" if num_classes > 2 else "binary"
    
    lgb = NativeLightGBMClassifier(
        featuresCol=x,
        labelCol=y,
        objective=objective,
        numLeaves=31,
    ...
    print(f"✅ Native LightGBM classifier created, fitting model...")
    lgbModel = lgb.fit(train)
    print(f"✅ Native LightGBM model fitted successfully")
...
class ModelBuilder:
    ...
    def __init__(self, spark_session: SparkSession):
        ...
        if LIGHTGBM_AVAILABLE:
            print("✅ Native LightGBM integration verified - adding to available models")
            self.model_types['lightgbm'] = {
                'class': NativeLightGBMClassifier,
                'model_class': NativeLightGBMModel,
    ...
    def build_model(self, train_data: DataFrame, features_col: str,
        ...
        if model_type not in self.model_types:
            ...
            if model_type == 'xgboost' and not XGBOOST_AVAILABLE:
                raise ImportError(f"XGBoost not available. Install with: pip install xgboost>=1.6.0")
            elif model_type == 'lightgbm' and not LIGHTGBM_AVAILABLE:
                raise ImportError(f"LightGBM not available. Install with: pip install synapseml>=0.11.0")
            else:
    ...
    def _build_model_with_hyperparams(self, train_data: DataFrame, features_col: str,
        ...
        if model_type == 'logistic':
            ...
            print(f"🔧 Building Native LightGBM model with hyperparameters: {hyperparams}")
            ...
            print(f"✅ Native LightGBM classifier created, training with {train_data.count()} samples...")
            lgb_model = lgb.fit(train_data)
            print(f"✅ Native LightGBM model training completed successfully")
    ...
    def create_estimator(self, features_col: str, label_col: str, model_type: str, **params) -> Any:
        ...
        if model_type == 'logistic':
            ...
            estimator = NativeLightGBMClassifier(
                featuresCol=features_col,
                labelCol=label_col,
                objective=objective,
                numLeaves=hyperparams.get('numLeaves', 31),
    ...
    def save_model(self, model: Any, path: str):
        ...
        if isinstance(model, NativeLightGBMModel):
            # Use custom save method for native LightGBM models
            model.save(path)
        else:
            # Use Spark ML save method for standard models
    ...
    def load_model(self, model_type: str, path: str) -> Any:
        ...
        if model_class is None:
            if model_type == 'xgboost':
                ...
                # For Native LightGBM, load as standalone model
                try:
                    model = NativeLightGBMModel.load(path)
                    print(f"Loaded {model_type} model from {path}")
                    return model
                except Exception as e:
                    print(f"Error loading Native LightGBM model: {str(e)}")
                    raise
            else:
