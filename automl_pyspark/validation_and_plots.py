...
def draw_ks_plot(output_dir, model_type):
    ...
    # Use xlsxwriter engine if available, otherwise use default
    excel_engine = 'xlsxwriter' if XLSXWRITER_AVAILABLE else 'openpyxl'
    writer = ExcelWriter(os.path.join(output_dir, 'KS_Charts.xlsx'), engine=excel_engine)
    ...
    for filename in found_files:
        try:
            ...
            for sheet_name in excel_file.sheet_names:
                ...
                # Apply conditional formatting only if xlsxwriter is available
                if XLSXWRITER_AVAILABLE:
                    ...
                    print(f"ℹ️ xlsxwriter not available - skipping conditional formatting for {sheet_name_final}")
...
def model_validation(output_dir, data, y, model, model_type, data_type):
    ...
    try:
        ...
        # Use xlsxwriter engine if available, otherwise use default
        excel_engine = 'xlsxwriter' if XLSXWRITER_AVAILABLE else 'openpyxl'
        decile_table.to_excel(decile_filepath, index=False, engine=excel_engine)
