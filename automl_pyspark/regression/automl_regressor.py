...
class AutoMLRegressor:
    ...
    def fit(self, 
        ...
        if isinstance(train_data, str):
            ...
            # Store filtered table reference if it was created
    ...
    def _fit_internal(self, train_data: DataFrame, target_column: str, 
        ...
        # Pass unified feature selection configuration to data processor
        ...
        self.data_processor.enable_rf_feature_selection = self.config.get('enable_rf_feature_selection', True)
        self.data_processor.min_size_for_rf_selection = self.config.get('min_size_for_rf_selection', 'medium')
        self.data_processor.rf_max_features = self.config.get('rf_max_features', self.config['max_features'])
        self.data_processor.rf_importance_threshold = self.config.get('rf_importance_threshold', 0.01)
        self.data_processor.output_dir = self.output_dir
        self.data_processor.user_id = self.config.get('user_id', 'default_user')
        self.data_processor.model_literal = self.config.get('model_literal', 'default_model')
        ...
        # Step 1.8: Feature Profiling (for all datasets after preprocessing)
        print("\n1.8. Feature Profiling for All Datasets...")
        self._perform_feature_profiling_all_datasets(
            train_df, oot1_df, oot2_df, 
            self.selected_vars, target_column
        )
        ...
        # Store trained models for SHAP computation
        ...
        if self.best_model_type is not None and hasattr(self, 'trained_models') and self.best_model_type in self.trained_models:
            try:
                import sys
                import os
                ...
                # Use the trained model from memory instead of trying to reload from disk
                best_model_obj = self.trained_models[self.best_model_type]['model']
    ...
    def _prepare_oot_datasets(self, oot1_data: Optional[DataFrame], 
        ...
        # This will be reused for optimization later in the method
    ...
    def _build_model_with_dual_training_cv(self, cv_data: DataFrame, target_column: str, 
        ...
        try:
            ...
            if len(default_cv_model.avgMetrics) > 0:
                default_metrics['cv_score'] = float(default_cv_model.avgMetrics[0])
            else:
                print(f"⚠️ Warning: avgMetrics is empty, using default score")
                default_metrics['cv_score'] = 0.0
