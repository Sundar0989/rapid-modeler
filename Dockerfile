...
# Install build tools (separate layer for better caching)
RUN apt-get update && apt-get install -y \
    build-essential \
    git \
    && rm -rf /var/lib/apt/lists/*
...
# Copy sample data files to working directory for Streamlit access
...
# Set Dataproc Serverless environment variables (disabled for local development)
ENV ENABLE_DATAPROC_SERVERLESS=false
...
ENV GCP_SERVICE_ACCOUNT_EMAIL=automl-dataproc-sa@atus-prism-dev.iam.gserviceaccount.com
ENV DATAPROC_SERVERLESS_ENABLED=false
